...
namespace dnnl {
namespace impl {
namespace primitive_hashing{ struct key_t; }
namespace kernel_cache{ struct key_t; }
namespace utils {
...
{
    T Data;
    bool IsNested = false, IsInsert = false;
    float Time = 0;
    constexpr CacheRecord(const T &data) noexcept : Data(data) {}
};
...
struct cache_t {
    ...
    using ExtraT = std::conditional_t<std::is_same_v<key_t, ::dnnl::impl::kernel_cache::key_t>, const char*, uint8_t>;
    std::deque<CacheRecord<ExtraT>> QueryRecords;
    ...
    bool& GetIsNested() const noexcept
    {
        thread_local bool IsNested = false;
        return IsNested;
    }
    CacheRecord<ExtraT>*& GetCurrent() const noexcept
    {
        thread_local CacheRecord<ExtraT> *Current = nullptr;
        return Current;
    }
    
    CacheRecord<ExtraT>& NewRecord(const key_t &key) noexcept
    {
        CacheRecord<ExtraT> *record = nullptr;
        {
            utils::lock_write_t lock_w(this->rw_mutex());
            if constexpr (std::is_same_v<key_t, ::dnnl::impl::primitive_hashing::key_t>)
                record = &QueryRecords.emplace_back(static_cast<uint8_t>(key.primitive_kind_));
            else if constexpr (std::is_same_v<key_t, ::dnnl::impl::kernel_cache::key_t>) 
                record = &QueryRecords.emplace_back(key.impl()->KernelName_);
            else
                record = &QueryRecords.emplace_back(uint8_t(0));
        }
        record->IsNested = GetIsNested();
        return *record;
    }
    ...
    cache_object_t get_or_create(const key_t &key, create_func_t create,
        ...
        if (!force_create && p_future.valid()) {
            ...
            record.Time = std::chrono::duration_cast<std::chrono::duration<float, std::milli>>(tend - tbegin).count();
...
protected:
    ...
    mutable utils::rw_mutex_t mutex;
    utils::rw_mutex_t &rw_mutex() const noexcept {
        return mutex;
    }
};
...
void PrintCache(const T& cache, const char* name, const S& stringify) noexcept
{
    using K = typename T::key_t;
    using E = typename T::ExtraT;
    struct Counter : public TimeCounter
    {
        uint32_t Create = 0, Nested = 0;
        constexpr void Add(float time, bool isNested, bool isInsert) noexcept 
        {
            TimeCounter::Add(time);
            Create += isInsert;
            Nested += isNested;
        }
        constexpr bool operator<(const Counter &rhs) const noexcept { return Count < rhs.Count; }
        constexpr float HitRate() const noexcept { return (Count - Create) * 100.f / Count; }
    };
    std::map<M, Counter> queryMap;
    Counter total;
    for (const auto &rec : cache.QueryRecords)
    {
        total.Add(rec.Time, rec.IsNested, rec.IsInsert);
        queryMap[static_cast<M>(rec.Data)].Add(rec.Time, rec.IsNested, rec.IsInsert);
    }
    printf("@@##%s Cache: size [%d/%d] query[%4u] create[%4u] hit[%6.2f%%] nested[%4u] time[%7.2f]ms\n",
        name, cache.get_size(), cache.get_capacity(), total.Count, total.Create, total.HitRate(), total.Nested, total.Time);
    std::vector<std::pair<M, Counter>> queries(queryMap.begin(), queryMap.end());
    std::sort(queries.begin(), queries.end(), [](const auto& lhs, const auto& rhs) { return rhs.second < lhs.second; });
    for (uint32_t i = 0; i < 5 && i < queries.size(); ++i) 
    {
        const auto [type, cnt] = queries[i];
        printf("--- [%-30s] query[%4u] create[%4u] hit[%6.2f%%] nested[%4u] time[%7.2f]ms\n",
            stringify(type), cnt.Count, cnt.Create, cnt.HitRate(), cnt.Nested, cnt.Time);
    }
}
